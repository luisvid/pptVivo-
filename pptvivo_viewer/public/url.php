...
try {
	...
	if (($proposedUrl != $request_uri [0]) && ($proposedUrl . '/' != $request_uri [0])) {
		...
		if (! isset ( $_SESSION ['s_languageIso'] ) || $_SESSION ['s_languageIso'] != $_REQUEST ['urlargs'] [0]) {
		...
		if (isset ( $_REQUEST ['action'] ) && $_REQUEST ['action'] == LoginActionManagerActions::LOGOUT) {
		...
		elseif (isset ($_REQUEST ['action'] ) && $_REQUEST ['action'] == LoginActionManagerActions::LOGOUTANDLOGIN) {
		...
		elseif (! isset ( $_SESSION ['loggedUser'] )) {
			...
			elseif (isset ( $_REQUEST ['action'] ) && $_REQUEST ['action'] != null && !isset ( $_REQUEST ['loginRequired'] )) {
