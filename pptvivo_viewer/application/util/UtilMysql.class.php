...
class UtilMysql {
	...
	/**
	 * Get SQL query to get specific content with concrete Lang.
	...
	public static function getMenuContents($languageId, $userTypeId, $startLevel, $endLevel = null) {
		...
		$query = 'select 
					...
					inner join
						usertypeaccess
						on usertypeaccess.menuid = menu.id and ( 
					';
		
	if($userTypeId != ''){						
		$query .= 'usertypeaccess.usertypeid = '.$userTypeId.' or';						
	}
		
		$query .= ' usertypeaccess.usertypeid is null
					)
	...
	public static function getCheckPermissions($userTypeId, $menuId) {
		...
		$query .= ' 
					usertypeaccess.usertypeid = '.$userTypeId.' or
				  ';
		}
		
		$query .= '
					(usertypeaccess.usertypeid is null)
					and menu.id = ' . $menuId;
