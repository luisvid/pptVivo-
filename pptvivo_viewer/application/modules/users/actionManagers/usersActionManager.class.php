	...
	public function delete() {
		...
		try {
			...
			if ($result) {
				...
				$_SESSION ['logger']->debug ( __CLASS__ . '-' . __METHOD__ . ' end' );
				$errorMessage = $_SESSION ['s_message'] ['error_deleting_user'];
				return new AjaxMessageBox ( ErrorMessageView::render ( $errorMessage ), null, Util::getLiteral('error') );
			}
		} catch ( Exception $e ) {
			$_SESSION ['logger']->error ( $_SESSION ['s_message'] ['error_deleting_user'] );
			$_SESSION ['logger']->debug ( __CLASS__ . '-' . __METHOD__ . ' end' );
	...
	public function create() {
		...
		try {
			...
			if (isset ( $_REQUEST ['userId'] ) && $_REQUEST ['userId'] != '') {
				...
				//Fix: This form doesn't request user name, but insertUser validates it.
				$_POST ['control_userName'] = $_POST ['control_userLogin'];
				...
				if ($result) {
					...
					if($resultData){
						...
						if($resultLoginType){
							...
							$_SESSION ['logger']->debug ( __CLASS__ . '-' . __METHOD__ . ' end' );
							return new AjaxRender(Util::getLiteral('register_user_viewer_success'));
					...
					$_SESSION ['logger']->debug ( __CLASS__ . '-' . __METHOD__ . ' end' );
					$errorMessage = $_SESSION ['s_message'] ['error_inserting_user'];
					return new AjaxMessageBox ( ErrorMessageView::render ( $errorMessage ), null, Util::getLiteral('error') );
				}
			...
			if (isset ( $_REQUEST ['userId'] ) && $_REQUEST ['userId'] != '') {
				$_SESSION ['logger']->error ( $_SESSION ['s_message'] ['error_updating_user'] );
				$_SESSION ['logger']->debug ( __CLASS__ . '-' . __METHOD__ . ' end' );
				$errorMessage = $_SESSION ['s_message'] ['error_updating_user'] . '<br />' . $e->getMessage();
				return new AjaxMessageBox ( ErrorMessageView::render ( $errorMessage ), null, Util::getLiteral('error') );
			} else {
				...
				$_SESSION ['logger']->error ( $_SESSION ['s_message'] ['error_inserting_user'] );
				$_SESSION ['logger']->debug ( __CLASS__ . '-' . __METHOD__ . ' end' );
				$errorMessage = $_SESSION ['s_message'] ['error_inserting_user'] . '<br />' . $e->getMessage();
				return new AjaxMessageBox ( ErrorMessageView::render ( $errorMessage ), null, Util::getLiteral('error') );
			}
		}
